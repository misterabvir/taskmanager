...
import { ALL, PROJECT_CREATE, GUID_EMPTY, PROJECT_GETALL, PROJECT_UPDATE_NAME, PAGE_PROJECT_DETAIL } from '../../../Utils/Const'
...
import { Get, Post } from '../../RequestsToServer/Requests'
...
export default class ProjectData extends Component<{}, ProjectDataState> {
  ...
  handleProjectNameChanged = async (value: string) => {
    await Post(PROJECT_UPDATE_NAME, { ProjectId: this.state.selectedProjectId, Name: value })
    await this.getProjects();
  };

  handleNewNameCreated = async (newProjectName: string) => {
    await Post(PROJECT_CREATE, { ProjectName: newProjectName })
    await this.getProjects();
  }
  ...
  renderContent = () => {
    ...
    const header = this.state.selectedProjectName === ALL ?
      ...
      <h3>Showed tasks for <a className='text-dark badge-pill' href={`${PAGE_PROJECT_DETAIL}/${this.state.selectedProjectId}`}>{this.state.selectedProjectName}</a> </h3>
    return (
      <div>
        {header}
  ...
  getProjects = async () => {
    ...
    const data = await Get(PROJECT_GETALL);
