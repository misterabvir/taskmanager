...
import { TASK_CREATE, PROJECT_DETAIL, PROJECT_UPDATE_NAME } from '../../../Utils/Const';
...
class ProjectDetail extends Component<ProjectDetailProps, ProjectDetailState>{
  ...
  handleProjectNameChange = async (name: string) => {
    await Put(PROJECT_UPDATE_NAME, { ProjectId: this.state.project?.projectId, Name: name });
  }

  handleNewTaskCreate = async (name: string) => {
    await Post(TASK_CREATE, { ProjectId: this.state.project?.projectId, TaskName: name },);
  ...
  getProjectDetail = async () => {
    const data = await Get(`${PROJECT_DETAIL}/${this.props.projectId}`);
